...
const npmDirectory = 'C:\\pake-build\\pake-cli';
...
function resolveIdentifier(url, explicitName, customIdentifier, author) {
    ...
    if (author) {
        ...
        const sanitizedName = (explicitName || 'pake-app').toLowerCase().replace(/[^a-z0-9]/g, '');
        return `com.${sanitizedAuthor}.${sanitizedName}`;
    }
...
async function mergeConfig(url, options, tauriConf) {
    ...
    const { appVersion, userAgent, showSystemTray, useLocalFile, identifier, name = 'pake-app', installerLanguage, wasm, camera, microphone, author, } = options;
...
async function handleOptions(options, url) {
    ...
    const appOptions = {
        ...options,
        name: resolvedName,
        identifier: resolveIdentifier(url, options.name, options.identifier, options.author),
    };
...
const EXTRA_STRING_KEYS = new Set(['name', 'title', 'identifier', 'author']);
